...
function parse( ml, maxwidth )
	...
	for i, blk in pairs( blocks ) do
		...
		if (blocks[i].text) then
				...
				elseif ( ch == "\t" ) then
					...
					if ( maxwidth and maxwidth > x ) then
						if ( xOffset + xSize + x >= maxwidth ) then
							...
							if ( lastSpacePos == string.utf8len( curString ) && lastSpacePos > 0 ) then
								...
								curString = string.utf8sub( curString, 1, sequenceStartPos - 1 )
							else
			...
			xSize = 0
		elseif(blocks[i].texture) then
			local newBlock = table.Copy(blocks[i])
				newBlock.colour = blocks[i].colour or {r = 255, g = 255, b = 255, a = 255}
				newBlock.thisX = blocks[i].w
				newBlock.thisY = blocks[i].h - 3
				newBlock.offset = {
					x = xOffset,
					y = yOffset
				}
			...
			xOffset = xOffset + blocks[i].w + 1
			texOffset = blocks[i].h/2
		end		
	end
