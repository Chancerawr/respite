	...
	if SERVER then
		if ( self.Owner:KeyDown( IN_ATTACK ) && self.Owner:WepRaised() && !self:GetDTBool(3) ) then
			if self.timeCooldown < CurTime() then
				...
				local endu = self.Owner:getChar():getAttrib("end")
				if stam < ( -1 * ((endu - 100)/10) * self.Primary.Stamina ) then return end
...
function SWEP:Swing()
	if SERVER then
		local current = self.Owner:getLocalVar("stm", 0)
		local endu = self.Owner:getChar():getAttrib("end")
		local new = (-1 * ((endu - 100)/10) * self.Primary.Stamina)
		self.Owner:setLocalVar("stm", current - new) -- 10 swings with 0 endurance
		self.Owner:getChar():updateAttrib("str", 0.002)	
	...
	local endu = self.Owner:getChar():getAttrib("end")
	local new = (-1 * ((endu - 100)/10) * self.Primary.Stamina)
	self.Owner:getLocalVar("stm", current - new) -- 10 swings with 0 endurance
	self.Owner:getChar():updateAttrib("str", 0.01)	
	--]]
