...
function ITEM:getDesc()
	...
	if (!self.entity or !IsValid(self.entity)) then
		str = "A device that allows you to send a simple signal to others far away.\nPower: %s\nFrequency: %s"
		return Format(str, (self:getData("power") and "On" or "Off"), self:getData("freq", "000.0"))
	else
		local data = self.entity:getData()
		
		str = "A device that allows you to send a simple signal to others far away. Power: %s Frequency: %s"
