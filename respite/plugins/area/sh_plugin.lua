...
nut.command.add("areainspect", {
	onRun = function(client, arguments)
		local areaID = client:getArea()

		if(areaID and client:isInArea(areaID)) then
			local areaData = PLUGIN.areaTable[areaID]
			
			if(areaData) then
				local desc = areaData.desc
				
				nut.chat.send(client, "area", desc)
			end
		else
			nut.chat.send(client, "area", "You are not in an area.")
		end
	end
})

--This is used for some entities to print stuff in the chat to people.
nut.chat.register("area", {
