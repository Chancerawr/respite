local PLUGIN = PLUGIN
PLUGIN.name = "Snow Replacement"
PLUGIN.author = "Ripped from Stormfox by Chancer"
PLUGIN.desc = "Replaces ground textures with snowy things when you want it to."

nut.config.add("snow", false, "Texture replacing map snow.", nil, {
	category = "Weather"
})
...
hook.Add("Think","StormFox - Snow Replacement",function()
	if(snowThink < CurTime()) then
		...
		local con = nut.config.get("snow", false)
		if (con) then
			MakeSnow(1)
			MakeSnow(1)
		else
