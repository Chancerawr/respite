...
local multiplier_walk_var = CreateClientConVar("nut_viewbob_mp_walk", 0.5, {FCVAR_CLIENTCMD_CAN_EXECUTE, FCVAR_ARCHIVE})
local multiplier_crouch_var = CreateClientConVar("nut_viewbob_mp_crouch", 0.45, {FCVAR_CLIENTCMD_CAN_EXECUTE, FCVAR_ARCHIVE})
local multiplier_sprint_var = CreateClientConVar("nut_viewbob_mp_sprint", 0.7, {FCVAR_CLIENTCMD_CAN_EXECUTE, FCVAR_ARCHIVE})
...
function PLUGIN:PlayerFootstep(client)
    ...
    if enable:GetFloat() >= 1 then
		...
		if step == 0 then step = 1 else step = 0 end
		if step == 0 then stepvalue = -1 else stepvalue = 1 end
