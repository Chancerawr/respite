...
function PANEL:loadBusinesses()
	for class, data in pairs(PLUGIN.zones) do
		...
		local onConfirm = function(newName)
			netstream.Start("zoneEdit", class, {name = newName})
			self:Close()
		end
		panel.OnMousePressed = function(this, code)
			...
			elseif (code == MOUSE_RIGHT) then
				...
				local menu = DermaMenu()
					...
					menu:AddOption("Zone Description", function()
						Derma_StringRequest("Zone Description", "Zone Description", data.desc, 
							function(text)
								netstream.Start("zoneEdit", class, {desc = text})
							end
						)
					end):SetImage("icon16/comment.png")
					menu:AddOption(L"moveToArea", function()
						netstream.Start("zoneTeleport", class)
...
vgui.Register("nutZoneManager", PANEL, "DFrame")

netstream.Hook("nutZoneManager", function(zones)
	PLUGIN.zones = zones
	zoneManager = vgui.Create("nutZoneManager")
end)
